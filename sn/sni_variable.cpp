...
namespace SNI
{
	...
	void SNI_Variable::ForEachCall(SNI_Cartesian * p_Cart, long p_Depth)
	{
		p_Cart->ProcessValueCall(SN::SN_Expression(this), NULL, p_Depth);
	}

	SN::SN_Error SNI_Variable::ForEachCartUnify(long p_Depth, SNI_Cart * p_Cart)
	{
		return p_Cart->ProcessValueUnify(p_Depth, this, NULL);
	}

	void SNI_Variable::ForEachUnify(SNI_Cartesian * p_Cart, long p_Depth)
	{
	...
	SN::SN_Error SNI_Variable::Unify(SN::SN_ParameterList * p_ParameterList, SN::SN_Expression p_Result)
	{
		if (m_Value)
		{
			...
			return l_clone->Unify(p_ParameterList, p_Result);
		}
		else
		{
	...
	/* static */ SN::SN_Expression SNI_Variable::AddLambdas(SN::SN_ParameterList * p_ParameterList, SN::SN_Expression p_Result)
	{
		if (p_ParameterList->size())
		{
			SN::SN_Expression param = p_ParameterList->at(0).GetValue();
			p_ParameterList->erase(p_ParameterList->begin());
			return SN::SN_Lambda(param, AddLambdas(p_ParameterList, p_Result));
		}
		return p_Result;
	}
