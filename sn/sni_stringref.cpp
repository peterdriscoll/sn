	...
	SN::SN_Value SNI_StringRef::DoEquals(SNI_Value *p_Other) const
	{
		...
		if (dynamic_cast<SNI_String *>(p_Other))
		{
			...
			if (!start.IsNullValue() && end.IsNullValue())
			{
				...
				l_ParameterList->push_back(skynet::False);
				l_ParameterList->push_back(endVar);
				l_ParameterList->push_back(SN::SN_Long((long) (start_pos + p_Other->GetString().length())));
			...
			if (start.IsNullValue() && !end.IsNullValue())
			{
				...
				SN::SN_ExpressionList *l_ParameterList = new SN::SN_ExpressionList();
				l_ParameterList->push_back(skynet::False);
				l_ParameterList->push_back(startVar);
				l_ParameterList->push_back((SN::SN_Long((long) (end_pos - p_Other->GetString().length()))));
				SNI_DelayedProcessor::GetProcessor()->Delay(skynet::Equals, l_ParameterList);
