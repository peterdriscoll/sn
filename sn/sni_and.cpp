...
namespace SNI
{
	...
	size_t SNI_And::CardinalityOfCall(long p_Depth, SN::SN_Expression * p_ParamList) const
	{
		...
		{
			if (p_ParamList[PC2_First].AllValuesEqual(skynet::False))
			{
				return 1;
			}
		}
		if (1 < p_Depth)
		{
			...
			{
				if (p_ParamList[PC2_Second].AllValuesEqual(skynet::False))
				{
					return 1;
				}
			}
	...
	size_t SNI_And::CardinalityOfUnify(long p_Depth, SN::SN_Expression * p_ParamList, long p_CalcPos, long p_TotalCalc) const
	{
		if (p_TotalCalc <= 2)
		{
			...
			{
			case PU2_Result:
				if (p_ParamList[PU2_First].IsKnownValue() && p_ParamList[PU2_First].AllValuesEqual(skynet::False))
				{
					return p_ParamList[PU2_First].Cardinality();
				}
				if (p_ParamList[PU2_Second].IsKnownValue() && p_ParamList[PU2_Second].AllValuesEqual(skynet::False))
				{
				...
				break;
			case PU2_First:
				if (p_ParamList[PU2_Result].IsKnownValue() && p_ParamList[PU2_Result].AllValuesEqual(skynet::False))
				{
					if (p_ParamList[PU2_Second].IsKnownValue() && p_ParamList[PU2_Second].AllValuesEqual(skynet::False))
					{
						return 0;
