...
namespace SNI
{
	...
	std::string SNI_NotEquals::GetTypeName() const
	{
		return "NotEquals";
	}

	std::string SNI_NotEquals::DisplayCpp() const
	{
		return "NotEquals";
	}

	std::string SNI_NotEquals::DisplaySN(long /*priority*/, SNI_DisplayOptions & /*p_DisplayOptions*/) const
	{
		return "!=";
	}
	...
	std::string SNI_NotEquals::GetOperator() const
	{
		return "!=";
	}
	/// @endcond

	bool SNI_NotEquals::IsKnownValue(const SN::SN_Expression & p_Param, long p_Index) const
	{
	...
	SN::SN_Value SNI_NotEquals::PrimaryFunctionValue(const SN::SN_Value &p_Left, const SN::SN_Value &p_Right) const
	{
		return p_Left.GetSNI_Value()->DoEquals(p_Right.GetSNI_Value()).GetSNI_Value()->DoNot().GetSNI_Value();
	}
	
	SN::SN_Error SNI_NotEquals::AssertValue(const SN::SN_Value &p_Left, const SN::SN_Value &p_Right, const SN::SN_Value &p_Result) const
	{
	...
	SN::SN_Value SNI_NotEquals::RightInverseFunctionValue(const SN::SN_Value &p_Result, const SN::SN_Value &p_Right) const
	{
		if (!p_Result.GetBool())
		{
			return p_Right;
	...
	SN::SN_Expression SNI_NotEquals::LeftInverseFunctionExpression(const SN::SN_Expression &p_Result, const SN::SN_Expression &p_Left) const
	{
		return skynet::Null;
	}

	SN::SN_Expression SNI_NotEquals::RightInverseFunctionExpression(const SN::SN_Expression &p_Result, const SN::SN_Expression &p_Right) const
	{
		return skynet::Null;
	}
	...
	SN::SN_Error SNI_NotEquals::PartialUnifyInternal(SN::SN_Expression &p_left, SN::SN_Expression &p_right, SN::SN_Expression &p_result)
	{
		...
		if (SN::Is<SNI_Value *>(left_value) && SN::Is<SNI_Value *>(right_value))
		{
			return SNI_Helpers::PartialAssertValue(p_result, left_value.GetSNI_Value()->DoEquals(right_value.GetSNI_Value()).DoNot());
		}
	...
	size_t SNI_NotEquals::CardinalityOfUnify(long p_Depth, SN::SN_Expression * p_ParamList, long p_CalcPos, long p_TotalCalc) const
	{
		if (p_TotalCalc <= 2)
		{
	...
	SN::SN_Error SNI_NotEquals::UnifyElement(long p_Depth, SN::SN_Expression * p_ParamList, SNI_World ** p_WorldList, long p_CalcPos, long p_TotalCalc, SNI_WorldSet * worldSet, const SNI_Expression *p_Source) const
	{
		...
		case 0:
		{
			...
			{
				return LOG_RETURN(context, AssertValue(p_ParamList[PU2_First].GetVariableValue(), p_ParamList[PU2_Second].GetVariableValue(), p_ParamList[PU2_Result].GetVariableValue().DoNot()));
			}
		}
		break;
		case 1:
		{
			...
			case PU2_First:
			{
				...
				{
					SN::SN_Value value = RightInverseFunctionValue(p_ParamList[PU2_Result].GetVariableValue().DoNot(), p_ParamList[PU2_Second].GetVariableValue());
		...
		case 2:
		{
			...
			else if (p_ParamList[PU2_Result].GetBool())
			{
				...
				{
					return p_ParamList[PU2_First].AddValue(RightInverseFunctionValue(p_ParamList[PU2_Result].GetVariableValue(), p_ParamList[PU2_Second].GetVariableValue()).DoNot(), p_Depth, p_WorldList, worldSet);
				}
				else if (p_ParamList[PU2_First].IsReferableValue() && !p_ParamList[PU2_First].IsKnownValue())
				{
					return p_ParamList[PU2_Second].AddValue(LeftInverseFunctionValue(p_ParamList[PU2_Result].GetVariableValue(), p_ParamList[PU2_First].GetVariableValue()).DoNot(), p_Depth, p_WorldList, worldSet);
				}
			}
		}
		}
