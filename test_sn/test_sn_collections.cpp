	...
	public:
		...
		TEST_METHOD(TestSet)
		{
			Initialize();
			{
				...
				(X.HasMember(Long(5))).Assert().Do();
				(!X.HasMember(Long(6))).Assert().Do();

				(Y.HasMember(Long(3))).Assert().Do();
				(Y.HasMember(Long(4))).Assert().Do();
				(Y.HasMember(Long(5))).Assert().Do();
				(!Y.HasMember(Long(6))).Assert().Do();
			}
